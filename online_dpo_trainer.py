...
class OnlineDPOTrainer(Trainer):
    ...
    def _generate(self, model, prompts):
        ...
        # Sample 2 completions per prompt of size `max_new_tokens` from the model  这里改成 G=8 次
        inputs = self._prepare_inputs(inputs)
        # prompt_ids = inputs["prompt_input_ids"].repeat(2, 1) # (2*B, L_prompt)
        # prompt_mask = inputs["prompt_attention_mask"].repeat(2, 1) # (2*B, L_prompt)
        prompt_ids = inputs["prompt_input_ids"].repeat(self.num_generations, 1) # (G*B, L_prompt)
        prompt_mask = inputs["prompt_attention_mask"].repeat(self.num_generations, 1) # (G*B, L_prompt)
        ...
        # 输出：(G*B, L_prompt + L_completion)

        completion_ids = output[:, prompt_ids.size(1) :] # (G*B, L_completion)
    ...
    def _forward(self, model, prompt_ids, prompt_mask, completion_ids, completion_mask):
        ...
        print(logits.log_softmax(dim=-1).shape, completion_ids.shape)
    ...
    def training_step(
        ...
        print("判断长度，应该等于batchsize",len(groundtruth))
        ...
        logprobs = self._forward(model, prompt_ids, prompt_mask, completion_ids, completion_mask)  # 看这里
        ...
        # ======= 注释掉原有 judge 和 reward model 相关逻辑 =======
        ...
        def get_rule_reward(completion, groundtruth):
            """
            completion: str 或 [{'role': ..., 'content': ...}]
            groundtruth: str
            """
            # 提取 completion 的文本内容
            ...
            if last_line.startswith("Answer:"):
                score += 0.1  # 格式正确
                answer = last_line[len("Answer:"):].strip()
                # 处理答案中的$符号和空格
                answer = answer.replace("$", "").strip()
                if answer == str(groundtruth).strip():
                    score += 0.9  # 答案正确
            return score
        ...
        # completions_grouped: batch_size x G
        completions_grouped = [
            [completions[j * batch_size + i] for j in range(G)]
            for i in range(batch_size)
        ]
        print("Completion group:", completions_grouped)
        # rewards_grouped: batch_size x G
        rewards_grouped = [
            [get_rule_reward(completion, groundtruth[i]) for completion in group]
            for i, group in enumerate(completions_grouped)
        ]
        print('rewards_grouped[0]的维度',len(rewards_grouped[0])) 
        print('rewards_grouped的维度',rewards_grouped[0])  
        rewards_tensor = torch.tensor(rewards_grouped, device=device)  # (batch_size, G)

        print('rewards_tensor',rewards_tensor)
        
        # 新增：根据 compare_strategy 选择 chosen/rejected
        compare_strategy = getattr(self, 'compare_strategy', 'max_min')  # 默认为 max_min
        if compare_strategy == 'max_min':
            chosen_indices_in_group = rewards_tensor.argmax(dim=1)  # (batch_size,)
            rejected_indices_in_group = rewards_tensor.argmin(dim=1)  # (batch_size,)
        elif compare_strategy == 'max_random':
            chosen_indices_in_group = rewards_tensor.argmax(dim=1)  # (batch_size,)
            # 对于每组，排除最大值下标后随机选一个
            rejected_indices_in_group = []
            for i in range(batch_size):
                candidates = list(range(G))
                best = chosen_indices_in_group[i].item()
                candidates.remove(best)
                rand_idx = np.random.choice(candidates)
                rejected_indices_in_group.append(rand_idx)
            rejected_indices_in_group = torch.tensor(rejected_indices_in_group, device=device)
        elif compare_strategy == 'max_max':
            chosen_indices_in_group = rewards_tensor.argmax(dim=1)  # (batch_size,)
            # 对于每组，找第二大的下标
            rejected_indices_in_group = []
            for i in range(batch_size):
                group_rewards = rewards_tensor[i]
                best = chosen_indices_in_group[i].item()
                group_rewards_clone = group_rewards.clone()
                group_rewards_clone[best] = float('-inf')
                second_best = group_rewards_clone.argmax().item()
                rejected_indices_in_group.append(second_best)
            rejected_indices_in_group = torch.tensor(rejected_indices_in_group, device=device)
        else:
            raise ValueError(f"Unknown compare_strategy: {compare_strategy}")

        print('chosen_indices_in_group',chosen_indices_in_group)
        print('rejected_indices_in_group',rejected_indices_in_group)

        # 计算全局下标（修正？）
        chosen_indices = chosen_indices_in_group + G * torch.arange(batch_size, device=device)  # (batch_size,)
        print('chosen_indices',chosen_indices)
        rejected_indices = rejected_indices_in_group + G * torch.arange(batch_size, device=device)  # (batch_size,)
        ...
        if self.args.loss_type == "sigmoid":
            losses = -F.logsigmoid(self.beta * logits)   # beta = 1/tau
        ...
        # loss = losses.mean()
        print("batchsize的维度",batch_size)
        print(losses)
        batch_size = losses.shape[0]
        isFull = self.buffer.add(losses)
        if isFull:
            print("IS FULL")
            losses = self.buffer.sample(batch_size)
        else:
            print("IS NOT FULL")
            losses = torch.zeros_like(losses, requires_grad=True)
        # print(losses) 
        loss = losses.mean()
        # Log everything
        if self.reward_model is not None:
            scores_margin = logits.mean()
            self.stats["objective/scores_margin"].append(scores_margin.item())
            self.stats["objective/scores"].append(logits.mean().item())
        ...
        self.stats["objective/kl"].append(mean_kl.item())
        non_score_reward = (-self.beta * kl).sum(1)
        mean_non_score_reward = non_score_reward.mean()
        self.stats["objective/non_score_reward"].append(mean_non_score_reward.item())
        if self.reward_model is not None:
            rlhf_reward = logits.mean() + non_score_reward
            self.stats["objective/rlhf_reward"].append(rlhf_reward.item())
        mean_entropy = -logprobs.sum(1).mean()
        self.stats["objective/entropy"].append(mean_entropy.item())
